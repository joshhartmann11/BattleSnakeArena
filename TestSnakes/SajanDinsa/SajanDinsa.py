...
class SajanDinsa():
    ...
    def find_path(self, board, start, end, trim_tip=False, prune_tip=False):
    ...
    def get_cut_path(self, board, start_pt, end_pt):
        board_f = deepcopy(board)
        cut_len = 0
        for cut_len in self.fibrange(board):
            tail_pt = board_f.prune_agent_tail(cut_len)
            path_candidate = self.find_path(board_f, start_pt, end_pt)
            if tail_pt and len(path_candidate) > 1:

                path_candidate = self.find_path(board_f, start_pt, tail_pt)
                break
        return path_candidate, cut_len, tail_pt


    def find_disjoint_path(self, board, snake, food):
        path_init = self.find_path(board, snake.head, food)

        path_return = self.find_path(board, food, snake.tip, trim_tip=True)
        ...
        disjoint_path = self.find_path(board, snake.head, food)
        if disjoint_path:
            path = disjoint_path
        else:
            path = path_init
    ...
    def get_longer_path(self, board, snake, cut_tail_pt, path_final, prune_tip=False):
        paths = []
        ...
        if snake.head.y - 1 >= 0 and board[snake.head.y - 1][snake.head.x] is None:
            paths.append(self.find_path(board, Point(snake.head.x, snake.head.y - 1), cut_tail_pt, prune_tip=prune_tip))
        ...
        if snake.head.x - 1 >= 0 and board[snake.head.y][snake.head.x - 1] is None:
            paths.append(self.find_path(board, Point(snake.head.x - 1, snake.head.y), cut_tail_pt, prune_tip=prune_tip))
    ...
    def closest_food(self, board):
        head = board.agent_snake.head
        dests = {board.pt_distance(head, f):f for f in board.food}
        return dests[min(dests.keys())]


    def get_move(self, data, board):
        snake = board.agent_snake
        food = self.closest_food(board)

        path_init, return_exists = self.find_disjoint_path(board, snake, food)
        ...
        if path_init and return_exists:
            ...
            cut_food_path, cut_food_len, food_pt = self.get_cut_path(board, snake.head, food)
            ...
            if cut_food_len < len(cut_food_path):
                ...
                path_food_longer = self.get_longer_path(board, snake, food_pt, cut_food_path)
                if cut_food_len < len(path_food_longer):
                    ...
                    cut_tip_path, cut_tip_len, cut_tip_pt = self.get_cut_path(board, snake.head, snake.tip)
                    path_tip_longer = self.get_longer_path(board, snake, cut_tip_pt, cut_tip_path, prune_tip=True)
        ...
        next_move = self.map_move(snake, Point(*path_final[1]))
